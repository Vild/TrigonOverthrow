...
void TransformComponent::recalculateMatrix() {
	...
	matrix = glm::translate(position) * glm::rotate(glm::radians(rotation.z), z) * glm::rotate(glm::radians(rotation.y), y) *
					 glm::rotate(-glm::radians(rotation.x), x) * glm::scale(scale);
}
