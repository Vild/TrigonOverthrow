...
void GunSystem::update(World& world, float delta) {
	for (std::unique_ptr<Entity>& entity : world.getEntities()) {
		...
		if (entity->getName() == "Player") {
			std::shared_ptr<GunComponent::RayGun> ragen = std::static_pointer_cast<GunComponent::RayGun>(gunComp->gun);
			ragen->ray.o = transform->position;
			ragen->ray.dir = transform->getDirection();
...
void GunSystem::testAgainst(std::unique_ptr<Entity>& target, HitboxComponent::HitboxType inType, std::shared_ptr<GunComponent::Gun> gun) {
	...
	case HitboxComponent::SPHERE: {
		...
		raygun->ray.t[0] = raygun->ray.o + t1 * raygun->ray.dir;
		gun = raygun;
