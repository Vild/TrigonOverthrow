...
TextRenderer::TextRenderer(TextFactory& factory, const std::string& text, bool isStatic, unsigned int maxTextLength)
	...
	_mesh
		->addBuffer("renderInfo",
								[&](GLuint id) {
									glBindBuffer(GL_ARRAY_BUFFER, id);
									glBufferData(GL_ARRAY_BUFFER, sizeof(CharRenderInfo) * _renderingData.size(), &_renderingData[0],
															 isStatic ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);

									glEnableVertexAttribArray(ShaderAttributeID::charRect);
									glVertexAttribPointer(ShaderAttributeID::charRect, 4, GL_FLOAT, GL_FALSE, sizeof(CharRenderInfo), NULL);
									glVertexAttribDivisor(ShaderAttributeID::charRect, 1);

									glEnableVertexAttribArray(ShaderAttributeID::charPos);
									glVertexAttribPointer(ShaderAttributeID::charPos, 2, GL_FLOAT, GL_FALSE, sizeof(CharRenderInfo), NULL);
