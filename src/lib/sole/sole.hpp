...
#endif
#define $windows $yes
#elif defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__) || defined(__MINT__) || defined(__bsdi__)
#include <ifaddrs.h>
#include <net/if_dl.h>
...
inline uint64_t rebase(const std::string& input, const std::string& basemap) {
	auto strpos = [](const std::string& chars, char ch) -> size_t { return chars.find_first_of(ch); };
...
inline std::string printftime(uint64_t timestamp_secs = 0, const std::string& locale = std::string()) {
	...
	try {
		...
		std::tm tm;
		$msvc(localtime_s(&tm, &t);) $melse(localtime_r(&t, &tm);)
	...
	uint64_t tv_nsec;
				 ...
				 inline int gettimeofday(struct timeval* tv, struct timezone* tz) {
					 ...
					 if (NULL != tz) {
						 ...
						 $msvc(_get_timezone(&timezoneSecs); _get_daylight(&daylight);) $melse(timezoneSecs = _timezone; daylight = _daylight;)
...
inline bool get_any_mac(std::vector<unsigned char>& _node) {
	$windows({
		...
		$bsd({
			...
			if (!foundAdapter)
				return $no("cannot determine MAC address (no suitable network adapter found)") false;
			return true;
		})
			...
			$osx({
				...
				$linux({
					...
					if (s == -1)
						return $no("cannot open socket") false;

					std::strcpy(ifr.ifr_name, "eth0");
					int rc = ioctl(s, SIOCGIFHWADDR, &ifr);
					close(s);
					if (rc < 0)
						return $no("cannot get MAC address") false;
					...
					$unix({
						...
						if (!pHost)
							return $no("cannot get host IP address") false;
...
namespace run {
auto epoch = []() { return std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()); };
